...
const users = ref([
    ...
    { id: 2, firstName: 'ณัฐวรรณ', lastName: 'เบเกอร์', status: 'ยืนยันสิทธิ์แล้ว', permission: 'นักศึกษา', major: 'วิทยาการคอมพิวเตอร์และปัญญาประดิษฐ์' },
    { id: 3, firstName: 'ณัฐพงศ์', lastName: 'พันธะลา', status: 'ยืนยันสิทธิ์แล้ว', permission: 'นักศึกษา', major: 'วิทยาการคอมพิวเตอร์และปัญญาประดิษฐ์' },
    { id: 4, firstName: 'พงศ์พล', lastName: 'ไชยชวลิตสกุล', status: 'ยืนยันสิทธิ์แล้ว', permission: 'นักศึกษา', major: 'วิทยาการคอมพิวเตอร์และปัญญาประดิษฐ์' },
    { id: 5, firstName: 'กฤติเดช', lastName: 'เกลอรัตนกุล', status: 'ยืนยันสิทธิ์แล้ว', permission: 'นักศึกษา', major: 'วิทยาการคอมพิวเตอร์และปัญญาประดิษฐ์' },
    { id: 6, firstName: 'โชคชัย', lastName: 'ไม่ใยดี', status: 'ยืนยันสิทธิ์แล้ว', permission: 'อาจารย์', major: 'วิทยาการคอมพิวเตอร์และปัญญาประดิษฐ์' },
]);
...
<template>
    ...
    <main class="bg-[#EBE8E8] flex justify-center items-center py-12 px-4 min-h-[calc(96vh-100px)]">
        <section class="w-full max-w-6xl">
            <div class="mb-6">
                <h1 class="text-2xl md:text-3xl font-bold mb-1">จัดการบัญชีผู้ใช้งาน</h1>
                <p>รายละเอียดข้อมูลสำหรับจัดการบัญชีผู้ใช้งาน</p>
            </div>
            ...
            <div class="bg-white p-4 md:p-6 lg:p-10 rounded-lg">
                ...
                <div class="lg:hidden space-y-4">
                    <div v-for="user in filteredUser" :key="user.id" 
                        ...
                        <div class="space-y-3">
                            ...
                            <!-- สาขาวิชา -->
                            <div class="flex justify-between items-center">
                                <p class="text-sm text-gray-500">สาขาวิชา</p>
                                <p class="text-sm font-medium text-right">{{ user.major }}</p>
                            </div>

                            <!-- ปุ่มจัดการ -->
                            <div class="pt-3">
                                <button class="w-full mb-3 bg-[#C4990D] text-white py-2.5 px-4 rounded cursor-pointer hover:bg-[#CEB668] transition-colors">
                                    แก้ไขสิทธิ์
                                </button>
                                <button class="w-full bg-red-600 text-white py-2.5 px-4 rounded cursor-pointer hover:bg-red-400 transition-colors">
                                    ลบบัญชี
                                </button>
                            </div>
                        </div>
                ...
                <div class="hidden lg:block bg-white shadow overflow-hidden">
                    <div class="overflow-x-auto">
                        <table class="min-w-[600px] w250 lg:w-full">
                            <!-- Table Header -->
                            <thead class="bg-[#2E70D3] text-white">
                                <tr>
                                    ...
                                    <th class="px-4 py-3 text-center text-base md:text-lg font-semibold">สาขาวิชา</th>
                            ...
                            <tbody class="bg-white">
                                ...
                                <tr v-for="(user, index) in filteredUser" :key="user.id"
                                    ...
                                    <td class="px-4 py-3 flex gap-2 text-center">
                                        <button class="bg-[#C4990D] cursor-pointer text-white py-2 px-6 rounded hover:bg-[#CEB668] transition-colors">
                                            แก้ไขสิทธิ์
                                        </button>
                                        <button class="bg-red-600 cursor-pointer text-white py-2 px-6 rounded hover:bg-red-400 transition-colors">
                                            ลบบัญชี
                                        </button>
                                    </td>
                                </tr>
